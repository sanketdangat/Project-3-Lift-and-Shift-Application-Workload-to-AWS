...
```sh
ssh -i vprofile-key.pem centos@<public_ip_of_instance>
sudo su -
curl http://169.254.169.254/latest/user-data
systemctl status mariadb

login into database & check the database
mysql -u root -p
show databases;
use accounts;
show tables;
...
mc01 172.31.88.98
```
- Create `sdangat.xyz` Private Hosted zone in Route53. we will pick `Default VPC` in `N.Virginia` region.
...
```sh
jdbc.url=jdbc:mysql://db01.sdangat.xyz:3306/accounts?useUnicode=true&

memcached.active.host=mc01.sdangat.xyz

rabbitmq.address=rmq01.sdangat.xyz
```
...
```sh
ssh -i "vprofile-key.pem" ubuntu@<public_ip_of_server>
sudo su -
...
- Next we will create a Launch configuration using the AMI created in above step for our ASG.
```sh
Name: vprofile-app-LT
AMI: vprofile-app-image
InstanceType: t2.micro
Additional configuration: Enable EC2 instance detailed monitoring within CloudWatch
